...
契約詳細の「やり取り履歴」→「記録」、または未連携履歴の「編集」で「スケジュール登録」をチェックすると、
...
予定を作成します。件名には契約番号と概要、本文には概要・詳細メモ・登録者・履歴への URL を転記するため、
...
一度予定登録を要求した履歴は、編集時も別の予定を追加登録できません。
...
| PUT | `/api/communications/{comm_id}` | 履歴を編集。未連携の履歴に限り、同じ `calendar` 指定で初回の予定登録が可能 |
...
「カレンダー」メニューで、自分の Outlook と共通グループの予定を日本時間の日・週・月表示で確認できます。
個人の予定本文の読み取りには、API アプリへ `Calendars.Read` の委任権限と管理者同意を追加してください。
履歴への URL は `APP_PUBLIC_URL`（HTTPS オリジン）で設定します。

ICS の URL 購読・ファイルを自分専用の表示ソースとして保存できます。URL は画面表示時に再取得し、
ファイルはアップロードした内容を再表示します。ICS は表示のみで Outlook には登録しません。
表示ソースの選択・色・表示形式もユーザー別に保存します。取得に失敗したソースだけにエラーを表示します。
暗号化キー・公開 HTTPS 購読の制限・サイズ上限は
[設定値リファレンス](../docs/CONFIGURATION.md#共通カレンダー連携)を参照してください。

| メソッド | パス | 説明 |
|---|---|---|
| GET | `/api/calendar/availability` | 自分・グループの連携設定状況（権限の最終判定は取得時） |
| GET | `/api/calendar/events?source=personal\|group&start=...&end=...` | 本人／固定グループの予定一覧。開始・終了はタイムゾーン付き日時 |
| GET / PUT | `/api/calendar/preferences` | 本人の表示設定 |
| GET / POST | `/api/calendar/feeds` | 本人の ICS ソース一覧／追加。ファイルは UTF-8 内容を JSON の `content` に指定 |
| PUT / DELETE | `/api/calendar/feeds/{id}` | 本人のソースの名前・色・表示選択の変更／削除 |
| GET | `/api/calendar/feeds/{id}/events?start=...&end=...` | 本人の ICS 予定を表示範囲に展開。Outlook への書き込みなし |
