...
未確定の予定は既定で非表示です。「未確定の予定を表示」で切り替えでき、この選択もユーザー別に保存します。
対象は Outlook（個人・グループ）の `showAs=tentative` と ICS の `STATUS:TENTATIVE` です。
未回答・仮承諾という招待への返答だけでは非表示にしません。保存する予定自体は除外せず、表示時に切り替えます。
初回表示で取得・解析に成功した ICS と予定を保存し、通常表示では保存済みデータを使用します。
URL の再取得は「更新」ボタンを押した場合だけ行います。表示期間の移動・再訪では URL に接続しません。
未表示の期間は保存済み ICS から解析します。同じ期間やその中の日・週表示は保存済みの解析結果を使用します。
保存データはユーザー別で、ソースごとに最新の ICS と直近 1 表示期間の解析結果だけを保持します。
更新成功時は保存内容を置き換えるため、購読元で削除された予定を履歴として残す機能ではありません。
ファイル内容や URL を差し替える場合は、そのソースを削除して追加し直してください。
ICS は表示専用で、Outlook への取り込み・予定作成は一切行いません。

購読 URL・ファイル内容・取得済み ICS・解析済み予定は `SENSITIVE_ENC_KEY` で暗号化保存します。
ソースを削除すると保存済みデータも削除します。ブラウザーの永続ストレージには保存しません。
...
更新時の取得・解析に失敗しても、正常な保存データを置き換えません。
保存済みの予定を表示し、取得失敗と最終正常取得日時を明示します。最新情報として扱うことはありません。
初回取得に失敗した場合や、保存データでも対象期間を解析できない場合はエラーを表示します。
GET `/api/calendar/feeds/{id}/events` は既定で保存データを使用し、`refresh=true` のときだけ再取得します。
...
元の繰り返し予定が ICS に含まれない個別の変更は、単独の予定として表示し「注意」を出します。
これは読み込み失敗ではなく、保存・表示は継続します。
