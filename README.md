...
- **オンプレミス設置が前提**です。認証には Entra ID を利用し、業務基盤は OSS で構成します。
  任意機能の共通カレンダー連携を有効にした場合は、Microsoft Graph 経由で Microsoft 365
  グループの予定表も利用します。
...
> 社内ホスト名・内部 IP・Proxmox 接続情報を含むため、
> Private リポジトリ **`MOUNTAIN-infra`** に分離して管理しています。
> （Proxmox 上への VM 自動作成、Docker 導入、nginx による TLS 終端・リバースプロキシ配置）
...
- **共通カレンダー連携（任意）** — やり取り履歴の登録・未連携履歴の編集時にチェックすると、別途指定した
  開始・終了日時で、管理者指定の Microsoft 365 グループ予定表へ予定を登録。
  本文に履歴 URL を付け、同じ履歴からの重複登録を防止。予定登録だけの失敗は履歴を残して再試行できます
- **カレンダー表示** — 自分・グループの Outlook 予定を日・週・月表示。
  自分専用の ICS URL 購読・ファイルを保存して重ね合わせ（ICS から Outlook への登録は行いません）
